...
By default, ~overleaf-authenticate~ stores cookies in Emacs auth-source.
...
~overleaf-read-cookies-from-file~ and ~overleaf-save-cookies-to-file~.
...
If ~overleaf-project-clone~, ~overleaf-project-config~,
~overleaf-project-push~, ~overleaf-project-push-force~, or
~overleaf-project-pull~ starts without valid cookies, the command asks
in the minibuffer whether it should run ~overleaf-authenticate~
immediately.  For cookies saved by ~overleaf-authenticate~, expiry is
...
- The old ~overleaf-project-sync~ and ~overleaf-project-sync-after-commit~
  commands remain as obsolete aliases for ~overleaf-project-push~ and
  ~overleaf-project-push-after-commit~.
